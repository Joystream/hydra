...
import { ExponentialBackOffStrategy, BackoffStrategy } from './BackOffStrategy'
...
export async function withTimeout<T>(
  ...
  return Promise.race([promiseFn, timeoutPromise]).then((x) => x as T)
}
...
export async function retry<T>(
  promiseFn: () => Promise<T>,
  retries = -1,
  backoff: BackoffStrategy = new ExponentialBackOffStrategy()
): Promise<T> {
  ...
  while (result === undefined && retriesLeft !== 0) {
    try {
      ...
      debug(`An error occured: ${JSON.stringify(
        e,
        null,
        2
      )}. Retrying in ${backoff.getBackOffMs()}ms. 
  ...
  backoff.resetBackoffTime()
  throw new Error(
    `Failed to resolve promise after ${retries} retries. Last error: ${logError(
      error
    )}`
  )
}
