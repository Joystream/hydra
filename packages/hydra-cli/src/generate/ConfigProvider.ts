import { Config } from '@metmirr/warthog'
...
export class ConfigProvider {
  ...
  constructor() {
    ...
    this.cliGeneratedPath = path.join(
      this.config.get('ROOT_FOLDER'),
      '/',
      this.config.get('CLI_GENERATE_PATH'),
      '/'
    )
  }
  ...
  withGeneratedFolderRelPath(name: string): GeneratorContext {
    ...
    const generatedFolderRelPath = path.relative(
      destFolder,
      this.config.get('GENERATED_FOLDER')
    )
    return {
