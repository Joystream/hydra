...
export default class Codegen extends Command {
  ...
  async run(): Promise<void> {
    ...
    this.parsedFlags.install =
      this.parsedFlags.install && process.env.HYDRA_NO_DEPS_INSTALL !== 'true'
    ...
    const generateFiles = {
      ...
      task: async () => {
        let indexFileContent = readFileSync(
          getTemplatePath('index-builder-entry.mst'),
          'utf8'
        )
        ...
        createFile(
          ...
          formatWithPrettier(indexFileContent)
        )

        let pkgJsonContent = readFileSync(
          getTemplatePath('indexer.package.json'),
          'utf8'
        )
        ...
        // Create
        await fs.copyFile(
          getTemplatePath('indexer.tsconfig.json'),
