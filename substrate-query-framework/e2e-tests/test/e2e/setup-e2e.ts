...
before(async () => {
  ...
  console.log(`Using Processor API endpoint ${process.env.PROCESSOR_ENDPOINT_URL}`);
  Container.set("ProcessorClient", new GraphQLClient(process.env.PROCESSOR_ENDPOINT_URL || ""));
  console.log(`Using Indexer API endpoint ${process.env.INDEXER_ENDPOINT_URL}`);
  Container.set("IndexerClient", new GraphQLClient(process.env.INDEXER_ENDPOINT_URL || ""));
});
